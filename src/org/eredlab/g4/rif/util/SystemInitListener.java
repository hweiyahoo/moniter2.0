...
public class SystemInitListener implements ServletContextListener {
   ...
   private void systemStartup(ServletContext servletContext) {
      ...
      if (success) {
         log.info("-------------------------------");
         log.info("进行业务系统初始化工作...");
         success = AppInit.AppInitStartup(servletContext);
      }
      long timeSec = (System.currentTimeMillis() - start) / 1000;
      log.info("********************************************");
      if (success) {
         log.info("下位机监控平台[NMS]启动成功[" + G4Utils.getCurrentTime() + "]");
         log.info("启动总耗时: " + timeSec / 60 + "分 " + timeSec % 60 + "秒 ");
      }
      else {
         log.error("下位机监控平台[NMS]启动失败[" + G4Utils.getCurrentTime() + "]");
   ...
   private void initDbType() throws SQLException {
      ...
      if (dbString.indexOf("ora") > -1) {
         ...
         if (log.isErrorEnabled()) {
            log.error(G4Constants.Exception_Head + "下位机监控平台[NMS]目前还不支持你使用的数据库产品.如需获得支持,请和我们联系!");
         }
         System.exit(0);
      }
   }

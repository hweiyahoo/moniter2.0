...
public class IdGenerator {
    ...
    * @return
    */
   public static String getDmOrgIdGenerator(String pParentid, String deptid) {
      if (G4Utils.isEmpty(deptid)) {
         deptid = "001";
      }
      else {
         // int intDeptId = Integer.valueOf(deptid).intValue();
         // String hexString = String.valueOf(intDeptId);
         if (deptid.length() == 1) {
            deptid = "00" + deptid;
         }
            ...
            log.error(G4Constants.Exception_Head + "生成直放站地区编号越界了.同级兄弟节点编号为[001-999]\n请和您的系统管理员联系!");
         }
         else {
            log.error(G4Constants.Exception_Head + "生成直放站地区编号发生未知错误,请和开发人员联系!");
         }
      }
      return pParentid + deptid;
   }

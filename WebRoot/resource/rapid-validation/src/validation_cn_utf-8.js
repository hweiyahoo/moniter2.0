...
Validator.messageSource['zh-cn'] = [
	...
	// 上位机 特定验证
	['validate-phone-all','请输入正确的电话号码或手机号码,如:[010[-]]29392929或13312345610,当前长度为%s.'],
	['unit1','1字节的无符号整数范围：0~255,当前长度为%s.'],
	['unit2','2字节的无符号整数范围：0~65535,当前长度为%s.'],
	['unit3','3字节的无符号整数范围：0~16777215,当前长度为%s.'],
	['unit4','4字节的无符号整数范围：0~4294967295,当前长度为%s.'],
	['sint1','1字节的有符号整数范围：-127~+127,当前长度为%s.'],
	['sint2','2字节的有符号整数范围：-32767~+32767,当前长度为%s.']
...
Validation.addAllThese([
	...
	// sunnada moniter system validation
	['validate-phone-all',/^(((0[1-9]{3})?(0[12][0-9])?[-])?\d{6,8}|([1][358][0-9]{9}))$/],
	['unit1',function(v,elm,args,metadata) {
		return (parseInt(v) >= 0 && parseInt(v) <= 255)
	},{depends : ['validate-integer']}],
	['unit2',function(v,elm,args,metadata) {
		return (parseInt(v) >= 0 && parseInt(v) <= 65535)
	},{depends : ['validate-integer']}],
	['unit3',function(v,elm,args,metadata) {
		return (parseInt(v) >= 0 && parseInt(v) <= 16777215)
	},{depends : ['validate-integer']}],
	['unit4',function(v,elm,args,metadata) {
		return (parseInt(v) >= 0 && parseInt(v) <= 4294967295)
	},{depends : ['validate-integer']}],
	['sint1',function(v,elm,args,metadata) {
		return (parseInt(v) >= -127 && parseInt(v) <= 127)
	},{depends : ['validate-integer']}],
	['sint2',function(v,elm,args,metadata) {
		return (parseInt(v) >= -32767 && parseInt(v) <= 32767)
	},{depends : ['validate-integer']}]
]);

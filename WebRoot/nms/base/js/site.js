...
Ext.onReady(function() {
   ...
   var grid = new Ext.grid.GridPanel({
      title      : '<span style="font-weight:normal">地区信息</span>',
   ...
   function editInitForCity() {
      ...
      cityEditWindow.show();
      cityEditWindow.setTitle('<span style="font-weight:normal">修改地市信息</span>');
   ...
   function addInitForCity() {
      ...
      cityEditWindow.show();
      cityEditWindow.setTitle('<span style="font-weight:normal">新增地市信息</span>');
   ...
   function addInitForProvince() {
      ...
      cityEditWindow.show();
      cityEditWindow.setTitle('<span style="font-weight:normal">新增省份信息</span>');
      Ext.getCmp('windowmode').setValue('add');
      Ext.getCmp('type').setValue(type);
      Ext.getCmp('btnReset').show();
      comboxWithTree.setDisabled(true);
   ...
   function editInitForProvince() {
      ...
      cityEditWindow.show();
      cityEditWindow.setTitle('<span style="font-weight:normal">修改省份信息</span>');
      Ext.getCmp('btnReset').show();
      comboxWithTree.setDisabled(true);
   }
   ...
   function deleteItemForProvince() {
      ...
      if (isProvince && pusecount > 0) {
         fields = fields + '[省级] ' + node.text + '<br>';
      }
      else if (!isProvince && cusecount > 0) {
         fields = fields + '[地市] ' + node.text + '<br>';
      }
      if (fields != '') {
         Ext.Msg.alert('提示', '<b>您选中地区在系统中正在使用</b><br>' + fields
                             + '<font color=red>正在使用不能删除!请重新选择！</font>');
         return;
      }
      var tipcontent = "";
      if (isProvince) {
         tipcontent = "你真的要删除选中的省级及其包含的所有地区吗？";
      }
      else {
         tipcontent = "你真的要删除选中的地区吗？"
      }

...
Ext.onReady(function() {
   ...
   var store = new Ext.data.Store({
      proxy  : new Ext.data.HttpProxy({
         url : './respflag.ered?reqCode=queryItems'
      }),
   ...
   formPanel = new Ext.form.FormPanel({
      ...
      items      : [{
         ...
         items  : [{
            ...
            items       : [{
               fieldLabel : '应答含意', // 标签
               name       : 'respname', // name:后台根据此name属性取值
               allowBlank : false,
               labelStyle : 'color:A62017;',
               anchor     : '100%'// 宽度百分比
            }]
      ...
      buttons       : [{
         ...
         handler : function() {
            ...
            addWindow.getComponent('addForm').form.submit({
               url       : './respflag.ered?reqCode=insertItem',
               waitTitle : '提示',
               method    : 'POST',
               waitMsg   : '正在处理数据,请稍候...',
      ...
      buttons       : [{
         ...
         handler : function() {
            ...
            editFormPanel.form.submit({
               url       : './respflag.ered?reqCode=updateItem',
               waitTitle : '提示',
               method    : 'POST',
               waitMsg   : '正在处理数据,请稍候...',
               success   : function(form, action) {
   ...
   function initEditWindow() {
      ...
      Ext.getCmp('respid_').setValue(record.get('respid'));
   }
   ...
   function deleteItems() {
      ...
      var strChecked = jsArray2JsString(rows, 'respid');
      var tipContent;
      tipContent = '您确定要删除吗？';
      Ext.Msg.confirm('请确认', tipContent, function(btn, text) {
         if (btn == 'yes') {
            showWaitMsg();
            Ext.Ajax.request({
               url     : './respflag.ered?reqCode=deleteItem',

...
Ext.onReady(function() {
   ...
   var MessageButton = new Ext.Button({
      ...
      scale      : 'medium',
      width      : 50,
      tooltip    : '<span style="font-size:12px">消息盒</span>',
      pressed    : true,
      arrowAlign : 'right',
      renderTo   : 'messageDiv',
      handler    : function() {
         // toastManager.send("您当前有【<font color='red'>1</font>】条重要警告！<a
         // href='javascript:showAlarmTabPanel(1953);'>查看</a>");
         // showAlarmTabPanel('1953');
         msgboxShowInit();
      }
   });
    ...
    * 消息盒显式初始化
    */
   function msgboxShowInit() {
      var msgboxShowWindow = new Ext.Window({
         id          : 'msgboxShowWindow',
         layout      : 'fit',
         width       : 350,
         height      : 200,
         ...
         // frame : false,
         autoLoad    : {
            url        : './msgbox.ered?reqCode=initForShowMsgbox&datetime=' + new Date().getTime(),
            text       : '引擎正在驱动页面,请等待...',
            discardUrl : false,
            nocache    : false,
            scripts    : true,
   ...
   function unlockSystem() {
      // showWaitMsg();
      if (!lockForm.form.isValid()) return;
...
function showTime() {
   ...
   s = s < 10 ? '0' + s : s;
   var rTimeObj = document.getElementById('rTime');
   if (rTimeObj != null) {
      rTimeObj.innerHTML = h + ":" + m + ":" + s;
   }

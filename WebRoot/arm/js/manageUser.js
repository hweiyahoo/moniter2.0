...
Ext.onReady(function() {
   ...
   grid.on("cellclick", function(grid, rowIndex, columnIndex, e) {
      ...
      if (fieldName == 'userid' && columnIndex == 2) {
         ...
         if (login_account == parent.DEFAULT_DEVELOP_ACCOUNT
            ...
            if (usertype == '1') {
               Ext.MessageBox
                             .alert('提示', '超级管理员和开发人员不能对业务经办员授权<br>' + '业务经办员只能被其所属部门或上级部门的管理员授予权限');
               return;
            }
         }
   ...
   var acceptCombo = new Ext.form.ComboBox({
      name           : 'accept',
      hiddenName     : 'accept',
      store          : new Ext.data.ArrayStore({
         fields : ['value', 'text'],
      ...
      value          : '0',
      fieldLabel     : '上报提醒',
      emptyText      : '是否接受上报提醒[告警、开站、修改、巡检]',
      allowBlank     : false,
      labelStyle     : micolor,
      forceSelection : true,
      editable       : false,
   ...
   var addUserFormPanel = new Ext.form.FormPanel({
      ...
      items       : [{
         ...
         allowBlank : false,
         anchor     : '99%'
      }, usertypeCombo, lockedCombo, sexCombo, acceptCombo, {
         fieldLabel : '联系电话1',
         name       : 'tel1',
         id         : 'tel1',
         allowBlank : true,
   ...
   function addInit() {
      ...
      Ext.getCmp('corp').setValue('三元达');
      Ext.getCmp('password').enable();
      Ext.getCmp('password1').enable();
      addUserWindow.show();
   ...
   var initPWDForm = new Ext.form.FormPanel({
      labelWidth  : 80,
      defaultType : 'textfield',
      labelAlign  : 'right',
      ...
      items       : [{
         ...
         xtype  : 'panel',
         border : false,
         html   : '<div style="font-size:12px;margin-left:10px">(提示:默认初始化密码是123456，您也可以自己重新设置)</div>'
      }]
   });

   var initPWDWindow = new Ext.Window({
      title       : '<span class="commoncss">密码初始化</span>',
      layout      : 'fit',
      width       : 360,
      height      : 130,
      closeAction : 'hide',
      ...
      listeners   : {
         'show' : function(obj) {
            initPWDForm.form.reset();
            initPWDForm.findById('password_init').setValue('123456');
            initPWDForm.findById('password_init').focus(true, 50);
         }
      },
      buttons     : [{
         text    : '保存',
   ...
   function updateUserPWD() {
      ...
      var tipcontent = '<font color=red>您确定要初始化以下用户登录密码：</font><br>' + fields + '</b>';
      Ext.Msg.confirm('请确认', tipcontent, function(btn, text) {
         if (btn == 'yes') {
            // return false;
            // showWaitMsg();
            var strChecked = jsArray2JsString(rows, 'userid');
            var password = initPWDForm.findById('password_init').getValue();
            Ext.Ajax.request({
               url     : './user.ered?reqCode=initUserPWD',
               ...
               failure : function(response, opts) {
                  Ext.Msg.alert('提示', '密码初始化异常，请稍后再试！', function() {
                     initPWDForm.form.reset();
                     initPWDForm.findById('password_init').focus();
                  });
               },
               params  : {
                  strChecked : strChecked,
